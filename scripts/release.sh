...
# "Deploying on a threshold"); a maintainer runs it for their own changes.
# Either way it is run from this repository's checkout — it releases the
# directory it sits in, so never from an installed copy — and not while a team
# is still working here. That wait is for the team, not for the commit: the
# commit below is `git add -A`, and .gitignore keeps .claude/worktrees/, where
# team worktrees live, out of it. It also keeps a worktree out of `git status`,
# so `git worktree list` is what shows a team still here. A user's own
